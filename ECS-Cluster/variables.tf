...
variable "load_balancer_internal" {
  ...
  type        = bool
} #Define se o Load Balancer deve ser interno (true) ou externo (false).

variable "load_balancer_type" {
  description = "O tipo de Load Balancer a ser criado (ex: 'application' ou 'network')."
  type        = string
} # O tipo de Load Balancer a ser criado (ex: 'application' ou 'network').
...
variable "node_instance_type" {
  description = "O tipo de instancia EC2 usada pelo ECS"
  type        = string
}
...
variable "cluster_on_demand_min_size" {
  description = "O número mínimo de instancias on demand"
  type        = number
}

variable "cluster_on_demand_max_size" {
  description = "O número máximo de instancias on demand "
  type        = number
}

variable "cluster_on_demand_desired_size" {
  description = "O número desejado de instancias on demand"
  type        = number
}
